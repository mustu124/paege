...
export function CartPageClient({ recommendedProducts }: CartPageClientProps) {
  ...
  if (items.length === 0) {
    return (
      <Container className="py-10 md:py-14">
        <h1 className="font-serif text-3xl italic text-charcoal-900 md:text-4xl">Cart Empty</h1>
        <div className="mt-10">
          <EmptyState
            title="Nothing to keep. Yet."
            description="We make limited numbers — pieces that matter."
            action={
              <Link href="/shop?filter=new-arrivals">
                <Button variant="primary">Shop The New Edit</Button>
              </Link>
            }
          />
        </div>
        ...
        {recommendedProducts.length > 0 && (
          <div className="mt-16">
            <h2 className="font-serif text-2xl italic text-charcoal-900">You Might Like</h2>
            <div className={`mt-8 ${PRODUCT_GRID_CLASSES}`}>
